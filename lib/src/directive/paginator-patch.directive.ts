...
export class PaginatorPatchDirective {
   ...
   */
  addPageIndex() {
    const pageIndex = this.matPaginator.getNumberOfPages();
    ...
    if (this.patch.list.length > pageIndex) {
      ...
      const hasActive = this.patch.list.find(({ active }) => active);
      // ? 点击最后一个?
    ...
    this.zone.onStable.pipe(take(1)).subscribe(() => {
      ...
      this.zone.run(() => {
        this.zone.onStable.pipe(take(1)).subscribe(() => {
          ...
          const nextBtn = this.hostElement.querySelector('.mat-paginator-navigation-next');
          // doc 插入到下一个按钮前面
